...
class Core_Library_File{
     ...
     * @param string $method 读写模式,默认模式为wb
     * @param bool $ifLock 是否锁文件，默认为true即加锁
     * @param bool $ifChmod 是否将文件属性改为可读写,默认为true
     * @return int 返回写入的字节数
     */
    public function write($fileName, $data, $method = "wb", $ifLock = true, $ifChmod = true) {
     ...
     */
    public function savePhpDataFile($fileName, $data, $isBuildReturn = true, $method = "rb+", $ifLock = true) {
        $temp = "<?php\r\n ";
